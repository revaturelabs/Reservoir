...
import com.revature.pkg.repository.BatchStateRepository;
...
import com.revature.pkg.util.BatchStates;
		...
		if (request.equals("update")) {
			...
			for (JsonElement element : jsonArray) {
				...
				if (checkBatch) {
					...
					boolean checkCurriculum = curriculumService
							.checkCurriculum(singleObject.get("curriculum").getAsString());
					if (!checkLocation) {
					...
					if (!checkCurriculum) {
						...
						boolean checkSkillset = skillsetService
								.checkSkillset(singleObject.get("curriculum").getAsString());
						if (!checkSkillset) {
							skillsetData.createSkillset(singleObject.get("curriculum").getAsString());
						}
						Integer skillsetId = skillsetService
								.getSkillsetInfo(singleObject.get("curriculum").getAsString()).get(0).getSkillSetId();
					...
					Integer getLocationId = locationService.getLocationInfo(singleObject.get("location").getAsString())
							.get(0).getLocationId();
					Integer getCurriculumId = curriculumService
							.getCurriculumInfo(singleObject.get("curriculum").getAsString()).get(0).getCurriculumId();
					@SuppressWarnings("unused")
					Integer getSkillsetId = skillsetService
							.getSkillsetInfo(singleObject.get("curriculum").getAsString()).get(0).getSkillSetId();
					...
					if (singleObject.has("programType")) {
						programType = singleObject.get("programType").getAsString();
					} else {
						programType = "null";
					}
					if (singleObject.has("interviewScoreLower")) {
						interviewScoreLower = singleObject.get("interviewScoreLower").getAsInt();
					} else {
						interviewScoreLower = 0;
					}

					Date dateStart = Date.valueOf(startDate);
					Date dateEnd = Date.valueOf(endDate);

					BatchState state = batchStateRepo.findByState(BatchStates.COMMITTED);
					Integer batchCapacity = 50;

					batchData.updateBatch(dateStart, dateEnd, state.getId(), interviewScoreLower, programType,
							getLocationId, getCurriculumId, BatchId, batchCapacity);
					...
					for (JsonElement trainer : trainers) {
						...
						boolean isEligible;
						if (singleTrainer.has("isEligible")) {
							isEligible = singleTrainer.get("isEligible").getAsBoolean();
						} else {
							isEligible = true;
						}
					...
					for (JsonElement associate : associates) {
						...
						boolean active;
						float score;
						if (singleAssociate.has("active")) {
							active = singleAssociate.get("active").getAsBoolean();
						} else {
							active = true;
						}
						if (singleAssociate.has("interviewScore")) {
							score = singleAssociate.get("interviewScore").getAsFloat();
						} else {
							score = 0;
						}
						...
						if (!checkAssociate) {
							associateData.addAssociates(associateFirstname, associateLastname, associateEmail, active,
									score, BatchId);
		...
		if (request.equals("add")) {
			...
			for (JsonElement element : jsonArray) {
				...
				if (!checkCurriculum) {
					...
					Integer skillsetId = skillsetService.getSkillsetInfo(singleObject.get("curriculum").getAsString())
							.get(0).getSkillSetId();
				...
				Integer getCurriculumId = curriculumService
						.getCurriculumInfo(singleObject.get("curriculum").getAsString()).get(0).getCurriculumId();
				@SuppressWarnings("unused")
				Integer getSkillsetId = skillsetService.getSkillsetInfo(singleObject.get("curriculum").getAsString())
						.get(0).getSkillSetId();
				String startDate = singleObject.get("startDate").getAsString();
				String endDate = singleObject.get("endDate").getAsString();
				String location = singleObject.get("location").getAsString();
				...
				batchData.createBatch(dateStart, dateEnd, state.getId(), interviewScoreLower, programType,
						getLocationId, getCurriculumId, batchCapacity);
				LOG.info("batch created ");
	...
	public void handleAssociate(JsonArray jsonArray, String request) {
		...
		if (request.equals("add")) {
			...
			for (JsonElement element : jsonArray) {
				...
				try {
					boolean active;
					if (singleObject.has("active")) {
						active = singleObject.get("active").getAsBoolean();
					} else {
						active = false;
					}
					boolean checkAssociate = associateService.checkAssociate(singleObject.get("email").getAsString());
					if (!checkAssociate) {
						associateData.addAssociateswithoutBatch(singleObject.get("firstName").getAsString(),
								singleObject.get("lastName").getAsString(), singleObject.get("email").getAsString(),
								active, singleObject.get("technicalScreeningScore").getAsFloat());
						counting++;
					} else {
	...
	public void handleSkill(JsonArray jsonArray, String request) {
		...
		if (request.equals("add")) {
			...
			for (JsonElement element : jsonArray) {
				JsonObject singleObject = element.getAsJsonObject();
				try {
					skillsdata.addSkill(singleObject.get("skillId").getAsInt(),
							singleObject.get("skillName").getAsString());
					counting++;
				} catch (Exception e) {
		...
		if (request.equals("delete")) {
			...
			for (JsonElement element : jsonArray) {
				JsonObject singleObject = element.getAsJsonObject();
				skillsdata.deleteSkill(singleObject.get("skillId").getAsInt(),
						singleObject.get("skillName").getAsString());
				counting++;
			}
